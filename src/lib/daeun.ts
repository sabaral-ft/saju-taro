...
const TWELVE_STAGE_ACTION_ADVICE: Record<TwelveStage, string> = {
  ...
  '제왕': '기회를 놓치지 말되, 겸손함을 잃지 마세요. 현재 맡고 있는 일에서 리더십을 발휘하고 성과를 극대화하세요. 단, 주변 사람들에게 감사를 표하고 독불장군이 되지 않도록 주의하세요.',
  ...
  '묘': '겉으로 드러나는 활동보다 저축, 공부, 자기 계발처럼 조용히 쌓아가는 일에 집중하세요. 장기적 관점에서 내실을 다지기에 좋습니다. 성급하게 결과를 기대하지 마세요.',
...
function generateDaeunDescription(
  ...
  if (isExtremeWeak) {
    ...
    // 60세 이상: 에너지 수준과 무관하게 건강 중심 조언
    if (rawStageData.energy >= 8) {
      desc += `운의 흐름이 좋아 활력이 넘치는 시기입니다. 건강이 허락하는 범위에서 취미 활동이나 봉사를 즐기세요. 가족과의 시간을 소중히 하세요.\n\n`;
    } else if (rawStageData.energy >= 5) {
      desc += `안정적인 흐름이니 건강 관리를 꾸준히 하면서 편안한 일상을 유지하세요. 무리하지 않는 것이 가장 큰 복입니다.\n\n`;
    } else {
      desc += `건강 관리에 각별히 신경 쓰는 시기입니다. 정기 검진을 빠뜨리지 말고, 가까운 사람들과의 교류를 유지하세요.\n\n`;
    }
  ...
  if (isExtremeWeak && weakOh) {
    ...
    const HEALTH_ADJUSTED_ADVICE: Record<Ohaeng, Record<string, string>> = {
      ...
      '수': {
        '묘': '장시간 앉아서 공부하기보다 50분 작업 후 반드시 10분 스트레칭을 하세요. 조용히 내실을 다지되, 앉아서 분석하는 시간이 길어지면 허리에 부담이 됩니다. 서서 일하는 습관을 들이세요.',
